...
echo ============================================
echo   MapSyncer Build: ALL 26.2
echo   (Fabric + NeoForge, protocol 776)
echo ============================================

findstr /r /b /c:"include 'mc-26.2:fabric'" "%SETTINGS_FILE%" >nul 2>&1
if %errorlevel% equ 0 (
    echo [1/5] Settings: already using 26.x isolated
) else (
    echo [1/5] Settings: switching to settings-26.gradle...
...
echo [2/5] Building mc-26.2:fabric...
call gradlew.bat :mc-26.2:fabric:clean :mc-26.2:fabric:build -x test
set FABRIC_RESULT=%errorlevel%

echo [3/5] Building mc-26.2:neoforge...
...
if %FABRIC_RESULT% neq 0 echo         Fabric 26.2: FAILED
if %NEO_RESULT% neq 0 echo         NeoForge 26.2: FAILED
...
call "%~dp0copy-release-jars.bat" mc-26.2\fabric\build\libs mc-26.2\neoforge\build\libs

echo.
echo ============================================
echo   BUILD SUCCESSFUL
echo ============================================
dir /b output\*-26.2*.jar 2>nul | findstr /v /i "-slim"
exit /b 0
